...
# Set up video writer to save the output depth video
...
output_video = cv2.VideoWriter('SA_Video_Depth_Map.avi', fourcc, fps, (output_width, output_height), False)
...
def objective_function(depth_map, edge_map):
    ...
    # Prioritize smoothness in non-edge areas
    smoothness = (1 - edge_map) * (np.var(grad_x) + np.var(grad_y))
...
while cap.isOpened():
    ...
    # Normalize the optimized depth map to a range from 0 to 255 for visualization
    optimized_depth_map = cv2.normalize(optimized_depth_map, None, 0, 255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)

    # Resize the optimized depth map to match the output video size
    optimized_depth_map_resized = cv2.resize(optimized_depth_map, (output_width, output_height))

    # Save optimized depth map to the video
    output_video.write(optimized_depth_map_resized)
